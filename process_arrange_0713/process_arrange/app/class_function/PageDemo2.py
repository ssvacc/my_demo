...
class PageDemo2(object):
    ...
    def create_demo2_page2(s_page_name, **kwargs):
        ...
        try:
            # 获取生成页面的配置信息
            config_result = PageDemo2.get_demo2_config_select_area_container_bak0706(s_page_name, **kwargs)
    ...
    # Tabs标签使用子标签
    @staticmethod
    def get_demo2_config_tabs_subElement(s_page_name, **kwargs):
        result = {"code": ReturnEnum.ER_FAIL().code, "msg": ReturnEnum.ER_FAIL().msg, "data": {}}
        # hq-container-select容器目前预期配置为三行，每行俩个元素
    ...
    # Tabs标签使用子组件
    @staticmethod
    def get_demo2_config_tabs_com(s_page_name, **kwargs):
        result = {"code": ReturnEnum.ER_FAIL().code, "msg": ReturnEnum.ER_FAIL().msg, "data": {}}
        # hq-container-select容器目前预期配置为三行，每行俩个元素
        a_page_template_config = [
            {'element': 'hq-tabs',
    ...
    # 搜索区使用容器配置
    @staticmethod
    def get_demo2_config_select_area_container_bak0706(s_page_name, **kwargs):
        result = {"code": ReturnEnum.ER_FAIL().code, "msg": ReturnEnum.ER_FAIL().msg, "data": {}}
        # hq-container-select容器目前预期配置为三行，每行俩个元素
        a_page_template_config = [
            {'element': 'hq-container-select',
             ...
             'sub_elements': [
                 {'sub_elements': [
                     ...
                     'page_area': ConstEnum.PAGE_CONFIG_INFO.HQ_TEMPLATE_TAG_LIST.value[1],
                     'start_field_name_en': 'i_count_start',
                     'end_field_name_en': 'i_count_end',
                 ...
                 {'sub_elements': [
                     {'element': 'hq-select-area-input-float',
                      ...
                      'page_area': ConstEnum.PAGE_CONFIG_INFO.HQ_TEMPLATE_TAG_LIST.value[1],
                      'start_field_name_en': 'i_price_start',
                      'end_field_name_en': 'i_price_end',
    ...
    # 搜索区使用容器配置
    @staticmethod
    def get_demo2_config_select_area_container_bak0704(s_page_name, **kwargs):
        result = {"code": ReturnEnum.ER_FAIL().code, "msg": ReturnEnum.ER_FAIL().msg, "data": {}}
        # hq-container-select容器目前预期配置为三行，每行俩个元素
        a_page_template_config = [
            {'element': 'hq-radio',
    ...
    @staticmethod
    def get_demo2_config2_bak0630(s_page_name, **kwargs):
        result = {"code": ReturnEnum.ER_FAIL().code, "msg": ReturnEnum.ER_FAIL().msg, "data": {}}
        # hq-container-select容器目前预期配置为三行，每行俩个元素
        a_page_template_config = [
            {'element': 'hq-radio',
