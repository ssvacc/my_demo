...
class GeneratePage(object):
    ...
    """
    @staticmethod
    def common_generate_page_by_config(a_page_template_config, s_page_name_en, a_flow_name = None, **kwargs):
        ...
        try:
            ...
            a_field_name_mark = []
            GeneratePage.set_repeat_mark(a_field_name_mark, a_page_template_config)

            # 遍历a_page_template_config，进行模板渲染，将容器组件中子标签的key抽取到一个list中
            o_vue_area = {"page_id": s_page_name_en, "page_name_en": s_page_name_en}
            ...
            content = {'ele': o_vue_area}
            main_rendered_string = render_to_string(template_name, content)
    ...
    """
    通用预览页面的接口函数,拿渲染好的数据解析拼接,用在跳转新窗口打开预览页面,新窗口不能调用系统接口
    参数: a_page_template_config: 页面配置信息，每个组件需有自己的组件模板名，page_area信息，组件key(对此次渲染唯一，用于容器识别渲染)
    返回值: s_vue_component_content 生成后的vue文件内容
           s_vue_component_file_name 生成后的vue文件名,存放在templates/output/
    """
    @staticmethod
    def common_view_page_by_config_new_window(a_page_template_config, s_page_name_en, a_flow_name = None, **kwargs):
        result = {"code": ReturnEnum.ER_FAIL().code, "msg": ReturnEnum.ER_FAIL().msg, "data": {}}
        try:
            # 容器组件的配置格式
            check_result = GeneratePage.check_container_config(a_page_template_config)
            ...
            # 最终模板渲染，预览文件
            content = {'ele': o_vue_area}
            html_name = PAGE_CONFIG_INFO.BASE_HTML_TEMPLATE.value
            main_rendered_html = str(render_to_string(html_name, content))
            result['code'] = ReturnEnum.ER_SUCCESS().code
            result['data']['s_vue_component_file_name'] = s_page_name_en
            result['data']['s_vue_component_content'] = main_rendered_html
            result['msg'] = ReturnEnum.ER_SUCCESS().msg
        except Exception as e:
            traceback.print_exc()
            result['code'] = ReturnEnum.ER_SERVER_ERROR().code
    ...
    @staticmethod
    def set_repeat_mark(a_field_name_mark, a_page_template_config):
        for config in a_page_template_config:
            if PAGE_CONFIG_INFO.HQ_COMMON_CONFIG_FIELD.value[1] in config.keys() and config[PAGE_CONFIG_INFO.HQ_COMMON_CONFIG_FIELD.value[1]]:
                ...
                if field_name_en in a_field_name_mark:
                    config[PAGE_CONFIG_INFO.HQ_COMMON_CONFIG_FIELD.value[0]] = field_name_en
                else:
                    a_field_name_mark.append(field_name_en)
            if GeneratePage.has_sub_element(config):
                subElements = config[PAGE_CONFIG_INFO.HQ_BASE_ELEMENT_KEY_LIST.value[2]]
                GeneratePage.set_repeat_mark(a_field_name_mark, subElements)
    ...
    def get_base_element_name(s_element_name):
        ...
        file_dir_list = PAGE_CONFIG_INFO.HQ_ELEMENT_FOLDER.value
        s_template_file_name = None
        for file_dir in file_dir_list:
            template_name = BASE_PATH + file_dir + os.sep + s_element_name + PAGE_CONFIG_INFO.TEMPLATE_FILE_SUFFIX.value
    ...
    def analysis_rendered_data(o_vue_area, template_rendered_string, config, parent_element_key):
        ...
        for s_tag_name in PAGE_CONFIG_INFO.HQ_TAG_LIST.value:
            if s_tag_name not in o_vue_area.keys():
                o_vue_area[s_tag_name] = ''
            if s_page_area not in o_vue_area.keys():
                o_vue_area[s_page_area] = ''
            if PAGE_CONFIG_INFO.HQ_DISTINCT_TAG.value not in o_vue_area.keys():
                o_vue_area[PAGE_CONFIG_INFO.HQ_DISTINCT_TAG.value] = ''
            # 各个区内容
            a_tag_content = GeneratePage.get_by_tag(template_rendered_string, s_tag_name)
            s_tag_content = a_tag_content[0]
            # 需要去重的标签进行去重
            s_tag_content = GeneratePage.distinct_tag_content(s_tag_content, o_vue_area,PAGE_CONFIG_INFO.HQ_DISTINCT_TAG.value)
            if s_tag_name == PAGE_CONFIG_INFO.HQ_TAG_LIST.value[0]:
                if not s_tag_content:
                    return
                if not parent_element_key:
                    # 不是容器的子组件，则将数据渲染到相应区块中
                    o_vue_area[s_page_area] += s_tag_content
                elif parent_element_key and parent_element_key in o_vue_area.keys() and o_vue_area[parent_element_key]:
            ...
            # 生命周期区
                ...
                if s_tag_content:
                    o_vue_area[s_tag_name] += s_tag_content
                    GeneratePage.analysis_sub_tag(o_vue_area, s_tag_content, PAGE_CONFIG_INFO.HQ_LIFE_CYCLE_TAG_LIST.value)
            else:
                o_vue_area[s_tag_name] += s_tag_content

    @staticmethod
    def distinct_tag_content(s_tag_content, o_vue_area, s_distinct_tag):
        if s_tag_content and s_distinct_tag in s_tag_content:
            # 包含应去重标签
            data_distinct_content = GeneratePage.get_by_tag(s_tag_content, s_distinct_tag)[0]
            # 标签和内容拼接
            data_distinct_content_tag = '<' + s_distinct_tag + '>' + data_distinct_content + '</'+ s_distinct_tag +'>'
            if data_distinct_content not in o_vue_area[s_distinct_tag]:
                o_vue_area[s_distinct_tag] += data_distinct_content
                s_tag_content = s_tag_content.replace(data_distinct_content_tag, data_distinct_content)
            else:
                s_tag_content = s_tag_content.replace(data_distinct_content_tag, '')
        return s_tag_content
    ...
    def check_container_config(a_page_template_config):
        ...
        for config in a_page_template_config:
            ...
            if PAGE_CONFIG_INFO.HQ_BASE_ELEMENT_KEY_LIST.value[0] in config.keys() and PAGE_CONFIG_INFO.HQ_CONFIG_AREA_LIST.value[1] == config[PAGE_CONFIG_INFO.HQ_BASE_ELEMENT_KEY_LIST.value[0]]:
